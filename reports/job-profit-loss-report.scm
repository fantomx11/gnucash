...
;; ------------------------------------------------------------------
(define-module (gnucash reports example my-sample-report))
...
(define (setup-query q owner accounts end-date job?)
  (let ((guid (gncOwnerReturnGUID (if job? owner (gncOwnerGetEndOwner owner))))
    ...
    (qof-query-add-guid-match
     q (list SPLIT-TRANS INVOICE-FROM-TXN INVOICE-BILLTO last-param)
     guid QOF-QUERY-OR)
    (qof-query-add-guid-match
     q (list SPLIT-LOT OWNER-FROM-LOT last-param)
     guid QOF-QUERY-OR)
    (qof-query-add-guid-match
     q (list SPLIT-LOT INVOICE-FROM-LOT INVOICE-BILLTO last-param)
     guid QOF-QUERY-OR)
...
(define (setup-query2 q owner accounts end-date invoice-type)
  (let ((guid (gncOwnerReturnGUID owner))
        (last-param QOF-PARAM-GUID))
    (qof-query-add-guid-match
     q (list SPLIT-TRANS INVOICE-FROM-TXN invoice-type last-param)
     guid QOF-QUERY-OR)
    (qof-query-add-guid-match
     q (list SPLIT-LOT OWNER-FROM-LOT last-param)
     guid QOF-QUERY-OR)
    (qof-query-add-guid-match
     q (list SPLIT-LOT INVOICE-FROM-LOT invoice-type last-param)
     guid QOF-QUERY-OR)
    (xaccQueryAddAccountMatch q accounts QOF-GUID-MATCH-ANY QOF-QUERY-AND)
    (xaccQueryAddDateMatchTT q #f end-date #t end-date QOF-QUERY-AND)
    (qof-query-set-book q (gnc-get-current-book))
...
(define (reg-renderer report-obj type)
  (define options1 (gnc:report-options report-obj))
  (define (opt-val1 section name)
    (gnc-optiondb-lookup-value options1 section name)) 
  (let* ((start-date (gnc:time64-start-day-time
                      (gnc:date-option-absolute-time
                       (opt-val1 gnc:pagename-general optname-from-date))))
         (end-date (gnc:time64-end-day-time
                    (gnc:date-option-absolute-time
                     (opt-val1 gnc:pagename-general optname-to-date))))
         (book (gnc-get-current-book))
         (date-format (gnc:options-fancy-date (gnc-get-current-book)))
         (owner-descr (owner-string type))
         (owner (opt-val1 owner-page owner-descr))
         (document (gnc:make-html-document))
         (report-title (string-append (G_ owner-descr) " " (G_ "Report"))))
    (gnc:html-document-add-object! document (gnc:html-markup-anchor "p" (gncOwnerIsValid owner)))

    (if (not (gncOwnerIsValid owner))
      (begin (gnc:html-document-add-object!
       document (gnc:html-make-generic-warning
                 report-title (gnc:report-id report-obj)
                 (cadr (get-info type)) (caddr (get-info type)))))

     (begin (gnc:html-document-set-headline!
        ...
        (render-revenues report-obj document type)
        (render-expenses report-obj document type)))

    document))



(define (render-expenses report-obj document type) 
  (define options2 (gnc:report-options report-obj))
  (define (opt-val2 section name)
    (gnc-optiondb-lookup-value options2 section name)) 
  (let*  ((start-date (gnc:time64-start-day-time
          (gnc:date-option-absolute-time (opt-val2 gnc:pagename-general optname-from-date))))
          (end-date (gnc:time64-end-day-time (gnc:date-option-absolute-time (opt-val2 gnc:pagename-general optname-to-date))))
          (book (gnc-get-current-book))
          (date-format (gnc:options-fancy-date (gnc-get-current-book)))
          (used-columns (build-column-used options2))
          (link-option (opt-val2 "Display Columns" linked-txns-header))
          (owner-descr (owner-string type))
          (date-type (opt-val2 gnc:pagename-general optname-date-driver))
          (owner (opt-val2 owner-page owner-descr))
          (acct-type ACCT-TYPE-PAYABLE)
    ...
    (setup-query2 query owner accounts end-date INVOICE-BILLTO)
    (let ((splits (qof-query-run query)))
      (qof-query-destroy query)
      (gnc:html-document-add-object! document (gnc:html-markup "h2" "Expenses:"))
      (cond
        ((null? splits)
          (gnc:html-document-add-object!
            document
                      ...
                      (when (> (length accounts-and-splits) 1)
                        (gnc:html-table-append-row!
                          table (gnc:make-html-table-cell/size
                            ...
                            (gnc:make-html-text
                              (gnc:html-markup-h3
                                (string-append (G_ "Account") ": " (xaccAccountGetName account)))))))

                      (add-owner-table table splits account start-date end-date date-type used-columns payable? link-option)))
                  accounts-and-splits))
...
(define (render-revenues report-obj document type) 
  (define options3 (gnc:report-options report-obj))
  (define (opt-val3 section name)
    (gnc-optiondb-lookup-value options3 section name)) 
 (let*  ((start-date (gnc:time64-start-day-time
          (gnc:date-option-absolute-time (opt-val3 gnc:pagename-general optname-from-date))))
          (end-date (gnc:time64-end-day-time (gnc:date-option-absolute-time (opt-val3 gnc:pagename-general optname-to-date))))
          (book (gnc-get-current-book))
          (date-format (gnc:options-fancy-date (gnc-get-current-book)))
          (used-columns (build-column-used options3))
          (link-option (opt-val3 "Display Columns" linked-txns-header))
          (owner-descr (owner-string type))
          (date-type (opt-val3 gnc:pagename-general optname-date-driver))
          (owner (opt-val3 owner-page owner-descr))
          (acct-type ACCT-TYPE-RECEIVABLE)
          (accounts (filter (lambda (a) (eqv? (xaccAccountGetType a) acct-type))
                      (gnc-account-get-descendants-sorted
                      (gnc-get-current-root-account))))
          (payable? (eqv? ACCT-TYPE-PAYABLE acct-type))
    ...
    (setup-query2 query owner accounts end-date INVOICE-OWNER)
    (let ((splits (qof-query-run query)))
      (qof-query-destroy query)
      (gnc:html-document-add-object! document (gnc:html-markup "h2" "Revenue:"))
      (cond
        ((null? splits)
          (gnc:html-document-add-object!
            document
...
(gnc:define-report
 'version 1
 'name (N_ "Job Profit/Loss")
 'report-guid "dce00e9bfc084b8c8ac4294c30d5fd64"
 'menu-name (N_ "Job Profit/Loss")
 'menu-tip (N_ "My options example report.")
